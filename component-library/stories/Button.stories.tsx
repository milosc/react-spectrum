...
import styles from './button-ripple.module.css';
import styles2 from './button-pending.module.css';
import './styles.css';
      ...
      {({isPending}) => (
        <>
          ...
          <ProgressBar
            aria-label="loading"
            isIndeterminate
            className={[styles2.spinner, (isPending ? styles2.spinnerPending : '')].join(' ')}>
...
function RippleButton(props) {
  ...
  return (
    <Button {...mergeProps(props, {onPress})} className={styles.rippleButton}>
      {isRippling ? (
        <span
